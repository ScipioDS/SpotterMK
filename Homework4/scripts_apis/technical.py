...
def fetch_data_from_db(code):
    username = 'test'
    password = 'Dians1233!'
    host = 'diansbaza.postgres.database.azure.com'
    port = '5432'
    database = 'postgres'

    # Correct SQLAlchemy format
...
def tech(code):
    ...
    majority_vote_combined = combined_decisions.mode()
    if not majority_vote_combined.empty:
        majority_vote_combined_value = majority_vote_combined[0]
    else:
        # Handle the case where no mode is found, for example:
        majority_vote_combined_value = None  # Or some default value
    print(f"Combined Decisions: {combined_decisions}")

    print(f"{majority_vote_combined}")
    return majority_vote_combined_value

@app.route('/technical', methods=['POST'])
def technical():
    data = request.json
    code = data.get('code')
    num_days = int(data.get('timeFrame'))
    if not code or not num_days:
        return jsonify({"error": "Code is required"}), 400
    return jsonify({"result": tech(code)}), 200
